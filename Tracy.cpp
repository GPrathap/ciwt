...
namespace tracy {
                ...
                if (draw_tracked_objects) {
                    ...
                    for (int i=0; i<visualization_3d_tracking_hypotheses.size(); i++) {
                        ...
                        if (hypo.bounding_boxes_3d().size()>0) {
                            ...
                            tracking_visualizer.RenderHypo3D(viewer, hypo, visualization_3d_left_camera, 0);
                        }

                        tracking_visualizer.RenderTrajectory(hypo, visualization_3d_left_camera, ("hypo_traj_3d_" + std::to_string(i)), r,g,b, viewer);
                    }
                    ...
                    if (draw_terminated_hypos) {
                        for (int i=0; i<visualization_3d_tracking_terminated_hypotheses.size(); i++) {
                            ...
                            tracking_visualizer.RenderTrajectory(hypo, visualization_3d_left_camera, ("hypo_term_traj_3d_" + std::to_string(i)), 0.0, 0.0, 1.0, viewer);
                        }
                    }
                }
    ...
    bool Tracy::ParseCommandArguments(const int argc, char **argv, po::variables_map &config_variables_map) {
    ...
    bool Tracy::RequestObjectProposals(int frame, po::variables_map &options_map,
    ...
    void Tracy::imageCallback(const sensor_msgs::ImageConstPtr& msg)
    {
        try
        {
        ...
        if (CIWTApp::tracking_mode_str == "detection_shape")  { // In detection-only mode, don't bother with proposals
            success_loading_proposals = CIWTApp::RequestObjectProposals(current_frame, variables_map,
                                                                        std::bind(GOT::segmentation::proposal_generation::ComputeSuppressedMultiScale3DProposals,
                                                                                  dataset_assistant.left_point_cloud_, dataset_assistant.left_camera_,
                                                                                  dataset_assistant.right_camera_, std::placeholders::_1),
...
int main(int argc, char** argv)
{
    ...
    printf("Tracking mode: %s.\r\n", tracy.tracking_mode_str.c_str());
    printf("Run tracker: %s.\r\n", tracy.run_tracker ? "YES" : "NO");

    // -------------------------------------------------------------------------------
    // +++ Globals +++
    // -------------------------------------------------------------------------------
    const int num_frames = tracy.end_frame-tracy.start_frame; // This many frames will be processed.

    // Makes sure the relevant values are correct.
    assert(num_frames>0);
    assert(tracy.debug_level>=0);
